// Fetch Heartstrings images (without collections/ prefix)
...
async function fetchHeartstringsImages() {
  try {
    console.log('Fetching Heartstrings Of Passion images...\n');
    
    const result = await cloudinary.api.resources({
      type: 'upload',
      prefix: 'HeartstringsOfPassion',
      max_results: 500,
      resource_type: 'image'
    });
    ...
    console.log('═══════════════════════════════════════════════════════════════');
    heroes.forEach(img => console.log(`  ${img.publicId} → ${img.size} (${img.ratio})`));

    console.log('\n═══════════════════════════════════════════════════════════════');
    console.log(`LOOKS: ${looks.length}`);
    console.log('═══════════════════════════════════════════════════════════════');
    looks.forEach(img => console.log(`  ${img.publicId} → ${img.size} (${img.ratio})`));

    console.log('\n\n═══ COPY THIS ═══\n');
    console.log(`const heartstringsImages = {
